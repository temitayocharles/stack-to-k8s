...
echo "🧹 Cleaning up Docker resources for Educational Platform..."
...
docker ps -a --filter "name=educational" --format "{{.ID}}" | xargs -r docker rm -f

# Remove unused images for this application
docker images --filter "reference=*educational*" --format "{{.ID}}" | xargs -r docker rmi -f

# Clean up unused volumes and networks
docker volume prune -f
docker network prune -f

echo "✅ Cleanup completed for Educational Platform!"
