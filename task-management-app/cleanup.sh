...
echo "🧹 Cleaning up Docker resources for Task Management application..."
...
docker ps -a --filter "name=task" --format "{{.ID}}" | xargs -r docker rm -f

# Remove unused images for this application
docker images --filter "reference=*task*" --format "{{.ID}}" | xargs -r docker rmi -f

# Clean up unused volumes and networks
docker volume prune -f
docker network prune -f

echo "✅ Cleanup completed for Task Management application!"
