...
echo "🧹 Cleaning up Docker resources for Medical Care System..."
...
docker ps -a --filter "name=medical" --format "{{.ID}}" | xargs -r docker rm -f

# Remove unused images for this application
docker images --filter "reference=*medical*" --format "{{.ID}}" | xargs -r docker rmi -f

# Clean up unused volumes and networks
docker volume prune -f
docker network prune -f

echo "✅ Cleanup completed for Medical Care System!"
