...
echo "🧹 Cleaning up Docker resources for this application..."
...
docker ps -a --filter "name=ecommerce" --format "{{.ID}}" | xargs -r docker rm -f

# Remove unused images for this application
docker images --filter "reference=*ecommerce*" --format "{{.ID}}" | xargs -r docker rmi -f

# Clean up unused volumes and networks
docker volume prune -f
docker network prune -f

echo "✅ Cleanup completed for E-commerce application!"
